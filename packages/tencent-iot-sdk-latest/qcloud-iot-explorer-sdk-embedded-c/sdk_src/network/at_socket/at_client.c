...
#include "qcloud_utils_param_check.h"
...
/*this function can be called only by at_uart_isr, just push the data to the
 * at_client ringbuffer.*/
 ...
 *        != 0: the response data will return when received setting lines number
 * data
...
int at_obj_exec_cmd(at_response_t resp, const char *cmd_expr, ...)
{
    ...
    if (client == NULL) {
        Log_e(
            "input AT Client object is NULL, please create or get AT Client "
            "object!");
...
#else
        if (0 == ring_buff_pop_data(client->pRingBuff, (uint8_t *)pch,
                                    1)) {  // push data to ringbuff @ AT_UART_IRQHandler
            continue;
        }
#endif
        else {
        ...
        if (at_recv_readline(client) > 0) {
            ...
            if ((urc = get_urc_obj(client)) != NULL) {
                ...
                if (resp_buf_len + client->cur_recv_len < client->resp->buf_size) {
                    ...
                    Log_e(
                        "Read response buffer failed. The Response buffer size is out "
                        "of buffer size(%d)!",

...
#include "qcloud_utils_aes.h"
#include "qcloud_utils_base64.h"
#include "qcloud_utils_hmac.h"
#include "qcloud_utils_httpc.h"
...
static int _get_json_result_code(char *json)
{
    ...
    if (LITE_get_int32(&result, v) != QCLOUD_RET_SUCCESS) {
        Log_e("Invalid json content: %s", json);
        HAL_Free(v);
        return -1;
    }

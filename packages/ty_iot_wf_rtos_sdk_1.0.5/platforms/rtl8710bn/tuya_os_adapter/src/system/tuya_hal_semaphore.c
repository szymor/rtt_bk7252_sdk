...
*  Date: 120427
***********************************************************/
static int InitSemaphore(IN const SEM_HANDLE semHandle,IN const uint32_t semCnt,\
                                 IN const uint32_t sem_max)
{
    if(!semHandle)
...
int tuya_hal_semaphore_create_init(SEM_HANDLE *pHandle, IN const uint32_t semCnt, \
                          IN const uint32_t sem_max)
{
    if(NULL == pHandle)
    {
...
*  Date: 120427
***********************************************************/
int tuya_hal_semaphore_wait(IN const SEM_HANDLE semHandle)
{
    if(!semHandle)
        return OPRT_INVALID_PARM;
...
*  Date: 120427
***********************************************************/
int tuya_hal_semaphore_post(IN const SEM_HANDLE semHandle)
{
    if(!semHandle)
        return OPRT_INVALID_PARM;
...
*  Date: 120427
***********************************************************/
int tuya_hal_semaphore_release(IN const SEM_HANDLE semHandle)
{
    if(!semHandle)
        return OPRT_INVALID_PARM;

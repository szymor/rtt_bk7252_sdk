...
#define DATA_TEMPLATE_CLOCK_THREAD_STACK_SIZE 1024 * 12
...
#define CLOCK_TOTAL_PROPERTY_COUNT     7
...
typedef struct _ClockProductDataDefine {
    ...
    TYPE_DEF_TEMPLATE_BOOL   m_time_mode;
    TYPE_DEF_TEMPLATE_BOOL   m_time_show_mode;
    TYPE_DEF_TEMPLATE_BOOL   m_temp_mode;
    TYPE_DEF_TEMPLATE_BOOL   m_voice;
    TYPE_DEF_TEMPLATE_STRING m_weather[CLOCK_MAX_STR_WEATHER_LEN];
    TYPE_DEF_TEMPLATE_FLOAT  m_temp;
    TYPE_DEF_TEMPLATE_FLOAT  m_humi;
    TYPE_DEF_TEMPLATE_STRING m_timer_set[CLOCK_MAX_STR_TIMER_SET_LEN + 1];
...
static void _init_data_template(void)
{
    ...
    clock_ProductData.m_time_mode = 0;
    clock_DataTemplate[0].data_property.key  = "time_mode";
    clock_DataTemplate[0].data_property.data = &clock_ProductData.m_time_mode;
    clock_DataTemplate[0].data_property.type = TYPE_TEMPLATE_BOOL;

    clock_ProductData.m_time_show_mode = 0;
    clock_DataTemplate[1].data_property.key  = "time_show_mode";
    clock_DataTemplate[1].data_property.data = &clock_ProductData.m_time_show_mode;
    clock_DataTemplate[1].data_property.type = TYPE_TEMPLATE_BOOL;

    clock_ProductData.m_temp_mode = 0;
    clock_DataTemplate[2].data_property.key  = "temp_mode";
    clock_DataTemplate[2].data_property.data = &clock_ProductData.m_temp_mode;
    clock_DataTemplate[2].data_property.type = TYPE_TEMPLATE_BOOL;

    clock_ProductData.m_voice = 0;
    clock_DataTemplate[3].data_property.key  = "voice";
    clock_DataTemplate[3].data_property.data = &clock_ProductData.m_voice;
    clock_DataTemplate[3].data_property.type = TYPE_TEMPLATE_BOOL;

    rt_memset(clock_ProductData.m_weather, 0, CLOCK_MAX_STR_WEATHER_LEN);
    clock_DataTemplate[4].data_property.key  = "weather";
    clock_DataTemplate[4].data_property.data = &clock_ProductData.m_weather;
    clock_DataTemplate[4].data_property.type = TYPE_TEMPLATE_STRING;

    clock_ProductData.m_temp = 0;
    clock_DataTemplate[5].data_property.key  = "temp";
    clock_DataTemplate[5].data_property.data = &clock_ProductData.m_temp;
    clock_DataTemplate[5].data_property.type = TYPE_TEMPLATE_FLOAT;

    rt_memset(clock_ProductData.m_timer_set, 0, CLOCK_MAX_STR_TIMER_SET_LEN);
    clock_DataTemplate[6].data_property.key  = "timer_set";
    clock_DataTemplate[6].data_property.data = &clock_ProductData.m_timer_set;
    clock_DataTemplate[6].data_property.type = TYPE_TEMPLATE_STRING;

    clock_ProductData.m_humi = 0;
    clock_DataTemplate[7].data_property.key  = "humi";
    clock_DataTemplate[7].data_property.data = &clock_ProductData.m_humi;
    clock_DataTemplate[7].data_property.type = TYPE_TEMPLATE_FLOAT;
};
/*-----------------data config end  -------------------*/
...
static void clock_deal_down_stream_user_logic(void *client, ClockProductDataDefine *clock)
{
    ...
    /* clock color */
    if (clock->m_time_mode)
    {
        HAL_Printf("clock->m_time_mode is %d\r\n", clock->m_time_mode);
    }else
    {
        HAL_Printf("clock->m_time_mode is %d\r\n", clock->m_time_mode);
    }
    ...
    /* time_show_mode */
    if (clock->m_time_show_mode)
    {
        HAL_Printf("clock->m_time_show_mode is %d\r\n", clock->m_time_show_mode);
    }else
    {
        HAL_Printf("clock->m_time_show_mode is %d\r\n", clock->m_time_show_mode);
    }
    ...
    /* temp_mode */
    if (clock->m_temp_mode)
    {
        HAL_Printf("clock->m_temp_mode is %d\r\n", clock->m_temp_mode);
    }else
    {
        HAL_Printf("clock->m_temp_mode is %d\r\n", clock->m_temp_mode);
    }
    ...
    /* timer_set */
    HAL_Printf("clock->m_timer_set is %s\r\n", clock->m_timer_set);

    if (eCHANGED == clock_get_property_state(&clock->m_time_mode)) {
#ifdef EVENT_POST_ENABLED
        if (clock->m_time_mode) {

...
static void _init_data_template(void)
{
    ...
    // clock_ProductData.m_power_switch = 0;
    // clock_DataTemplate[4].data_property.key  = "power_switch";
    // clock_DataTemplate[4].data_property.data = &clock_ProductData.m_power_switch;
    // clock_DataTemplate[4].data_property.type = TYPE_TEMPLATE_BOOL;
...
static void clock_deal_down_stream_user_logic(void *client, ClockProductDataDefine *clock)
{
    ...
    switch (clock->m_weather) {
        case eWeather_SUNNY:
            ...
            break;
        case eWeather_CLOUD:
            ansi_weather_name = "CLOUD";
            break;
        case eWeather_SHADE:
            ansi_weather_name = "SHADE";
            break;
        case eWeather_THUNDER_RAIN:
            ansi_weather_name = "THUNDER RAIN";
            break;
        case eWeather_LIGHT_RAIN:
            ansi_weather_name = "LIGHT RAIN";
            break;
        case eWeather_MODERATE_RAIN:
            ansi_weather_name = "MODERATE RAIN";
            break;
        case eWeather_HEAVY_RAIN:
            ansi_weather_name = "HEAVY RAIN";
            break;
        case eWeather_STORM_RAIN:
            ansi_weather_name = "STORM RAIN";
            break;
        default:
    ...
    if (clock->m_voice) {
        ...
        HAL_Printf( ">>>>>>>>>>>>>>>>>>>>>>>>> [  clock's voice is on  ]|[voice:%d]|[temp:%d]|[humi:%d]|[weather:%s]\n", \
                    clock->m_voice, clock->m_temp, clock->m_humi, ansi_weather_name);
        user_get_connect_status()->user_data.voice = clock->m_voice;
    } else {
        /* clock is off */
        HAL_Printf( ">>>>>>>>>>>>>>>>>>>>>>>>> [  clock's voice is off ]|[voice:%d]|[temp:%d]|[humi:%d]|[weather:%s]\n", \
                    clock->m_voice, clock->m_temp, clock->m_humi, ansi_weather_name);
        user_get_connect_status()->user_data.voice = clock->m_voice;
    }
...
static void clock_cycle_report(Timer *reportTimer)
{
    ...
    if (RT_TRUE == user_get_connect_status()->report_status)
    {
        clock_ProductData.m_voice = user_get_connect_status()->user_data.voice;
        clock_ProductData.m_temp = user_get_connect_status()->user_data.temp;
        clock_ProductData.m_humi = user_get_connect_status()->user_data.humi;
        clock_ProductData.m_weather = user_get_connect_status()->user_data.weather;
        if (expired(reportTimer)) {
            for (i = 0; i < CLOCK_TOTAL_PROPERTY_COUNT; i++) {
                clock_set_propery_state(clock_DataTemplate[i].data_property.data, eCHANGED);
                countdown_ms(reportTimer, 5000);
